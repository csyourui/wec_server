...
// NewBloomFilter creates and returns a new bloom filters using Redis as a backend.
...
k = round((m / n) * log(2));
*/
func NewBloomFilter(st StorageType, client *redis.ClusterClient, key string, n uint, p float64, cacheSize uint) (*BloomFilter, error) {

	bloom := &BloomFilter{key, n, p, nil, nil, &sync.RWMutex{}}
	if cacheSize != 0 {
		bloom.cache = NewKeyCache(cacheSize)
	}
	// calculate m and k with n and p
	m, k := EstimateParameters(n, p)
	partitionSize := math.Ceil(float64(m) / float64(k))
	//st types should Synchronization with StorageType
	switch st {
	case Redis:
		var err error
		bloom.storage, err = NewRedisStorage(client, bloom, key, k, uint(partitionSize))
		if err != nil {
			return nil, err
		}
	default:
...
// Exists checks if the given value is in the bloom filters or not. False positives might occur.
